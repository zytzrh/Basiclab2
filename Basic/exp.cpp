...
string ConstantExp::toString() {
   return integerToString(value);//ÒÑÓÐº¯Êý
}
...
bool IdentifierExp::hasequation() {
	return false;
}
/*
...
int CompoundExp::eval(EvalState & state) {
   ...
   if (op == "/" && right == 0) error("DIVIDE BY ZERO");
   error("SYNTAX ERROR");
   return 0;
}
...
bool CompoundExp::hasequation() {
	if (lhs->hasequation() || rhs->hasequation() || op == "=" || op == ">" || op == "<") return true;
	return false;
}